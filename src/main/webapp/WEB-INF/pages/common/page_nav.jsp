...
<div id="page_nav">
    <c:if test="${requestScope.books.pageNum>1}">
        <a href="${requestScope.url}1">首页</a>
    ...
    <c:forEach begin="${begin}" end="${end}" var="i">
        ...
        <c:if test="${requestScope.books.pageNum!=i}">
            <a href="${requestScope.url}${i}">${i}</a>
        </c:if>
    </c:forEach>

    <%--分页条页码输出的结束--%>
    <c:if test="${requestScope.books.pageNum<requestScope.books.pages}">
        <a href="${requestScope.url}${requestScope.books.pageNum +1}">下一页</a>
        <a href="${requestScope.url}${requestScope.books.total }">末页</a>
    </c:if>

    共${ requestScope.books.pages }页，${ requestScope.books.total }条记录
    到第<input value="${requestScope.books.pageNum}" name="pn" id="pn_input"/>页
