<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>购物车</title>
    <%@include file="/WEB-INF/pages/common/head.jsp"%>
    <script type="text/javascript">
        $(function () {
            $("a.delete").click(function () {
                return confirm("确定要删除《  " + $(this).parent().parent().find("td:first").text() + "  》吗？");
            });

            $("a.clear").click(function () {
                return confirm("确定要清空购物车吗？");
            });

            $(".updateCount").change(function () {
                var name = $(this).parent().parent().find("td:first").text();
                var id = $(this).attr("bookId");
                var count = this.value;

                if (confirm("确定要将商品《" + name + "》的数量修改为：" + count + "吗？")) {
                    location.href = "updateCount?count=" + count + "&name=" + name;
                } else {
                    this.value = this.defaultValue;
                }
            });
        });
    </script>


</head>
<body>

<div id="header">
    <img class="logo_img" alt="" src="static/imgs/logo.gif">
    <span class="wel_word">购物车</span>

    <%@include file="/WEB-INF/pages/common/login_success_menu.jsp" %>

</div>

<div id="main">

    <c:if test="${empty sessionScope.cart}">
        <%--如果购物车空的情况--%>
        <table>
            <tr>
                <td></td>
                <td></td>
                <td></td>
                <td></td>
                <td></td>
            </tr>
        <tr>
            <td colspan="5"><a href="index.jsp">亲，当前购物车为空！快跟小伙伴们去浏览商品吧！！！</a></td>
        </tr>
        </table>
    </c:if>

    <c:if test="${not empty sessionScope.cart}">
        <%--如果购物车非空的情况--%>
        <table>
            <tr>
                <td>商品名称</td>
                <td>数量</td>
                <td>单价</td>
                <td>金额</td>
                <td>操作</td>
            </tr>
        <c:forEach items="${sessionScope.cart}" var="cart">
            <tr>
                <td>${cart.name}</td>
                <td>
                    <input class="updateCount" style="width: 80px"
                           bookId="${cart.id}"
                           type="text"
                           value="${cart.count}">
                </td>
                <td>${cart.price}</td>
                <td>${cart.totalPrice}</td>
                <td><a class="delete" href="cart/delete/${cart.id}">删除</a></td>
            </tr>
        </c:forEach>
    </table>


    <div class="cart_info">
        <span class="cart_span">购物车中共有<span class="b_count">${sessionScope.totalCount}</span>件商品</span>
        <span class="cart_span">总金额<span class="b_price">${sessionScope.totalCountPrice}</span>元</span>
        <span class="cart_span"><a class="clear" href="cart/clear">清空购物车</a></span>
        <span class="cart_span"><a href="createOrder">去结账</a></span>
    </div>
    </c:if>
</div>


<%@include file="/WEB-INF/pages/common/footer.jsp" %>
</body>
</html>